...
class Snake:
	...
	def __init__(self, apple, iterations, graph):
	...
	#print score and reset
	def endGame(self):
		printScore(self)
		self.graph.plotPoint(self.iteration, self.getScore(), self.iterationLimit)
		self.reset()
		self.iteration += 1
...
def printScore(snake):
	print("Iteration:{}, Score:{}".format(snake.iteration, snake.getScore()))
...
class Graph:
	...
	def __init__(self, windowHeight, windowWidth, iterations):
		self.windowHeight = windowHeight
		self.windowWidth = windowWidth
		self.win = GraphWin('Results', self.windowHeight, self.windowWidth)
		self.drawAxis(iterations)
		
	def drawAxis(self, iterations):
		self.win.setBackground(color_rgb(255,255,255))
		self.win.setCoords(0,0,windowWidth,windowHeight)
		Xaxis = Line(Point(cellSize, cellSize), Point((columnCount - 1)* cellSize, cellSize))
		Xaxis.draw(self.win)
		Yaxis = Line(Point(cellSize, cellSize), Point(cellSize, (rowCount - 1) * cellSize))
		Yaxis.draw(self.win)

		Xlabel = Text(Point(300, 25), "Number of Iterations")
		Xlabel.draw(self.win)
		Ylabel = Text(Point(25, 550), "Score")
		Ylabel.draw(self.win)
		
		#x-axis
		for i in range(9):
			x, y = cellSize*(i+1), cellSize
			line = Line(Point(x,y), Point(x,y-6))
			line.draw(self.win)
			number = int(iterations /8 * i)
			increment = Text(Point(x, y-10), "{}".format(number))
			increment.setSize(6)
			increment.draw(self.win)
			
		#y-axis
		for i in range(9):
			x,y = cellSize, cellSize*(i+1)
			line = Line(Point(x,y), Point(x-5,y))
			line.draw(self.win)	
			number = 5*i
			increment = Text(Point(x-9, y), "{}".format(number))
			increment.setSize(6)
			increment.draw(self.win)	

	def plotPoint(self, x, y, iterations):
		x, y = x  * ((columnCount - 2) * cellSize / iterations)+ cellSize, y * ((columnCount - 2) * cellSize / 40) + cellSize
		point = Circle(Point(x, y), 2)
		point.setFill(color_rgb(0,0,0))
		point.draw(self.win)
...
def main():
	...
	#plotting results
	graph = Graph(windowHeight, windowWidth, args.iterations)
	...
	# initializes snake
	snake = Snake(apple, args.iterations, graph)
	snake.drawSnake(win)
